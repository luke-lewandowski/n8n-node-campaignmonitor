	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				default: '',
				description: 'Email address to where transactional email will be CC to.'
			},
			{
			...
			try {
				if (resource === 'member') {
					...
					if (operation === 'create') {
						...
						const restartSubscriptionBasedAutoresponders = this.getNodeParameter(
						...
						if (!jsonActive) {
							const mergeFieldsValues = (this.getNodeParameter(
								'mergeFieldsUi',
								i
							) as IDataObject).mergeFieldsValues as IDataObject[];
				...
				if (resource === 'transactional') {
					...
					if (operation === 'send') {
						...
						const smartEmailFields = (this.getNodeParameter(
							'smartEmailFields',
							i
						) as IDataObject).smartEmailFieldsValues as IDataObject[];
						if (smartEmailFields) {
							for (let i = 0; i < smartEmailFields.length; i++) {
								//@ts-ignore
								body.Data[
									smartEmailFields[i].name as string
